#!/usr/bin/env python3
"""
Quick start script for BiT (Big Transfer) federated transfer learning on CIFAR-100
with Opacus' make_private_with_epsilon API (epsilon-target DP on LoRA A matrices).
"""
...
try:
    ...
    if gpus:
        try:
            ...
            print("✅ TensorFlow GPU memory limited to 2GB, growth enabled")
...
def comprehensive_grad_sample_cleanup(model, verbose=False):
    ...
    for name, param in model.named_parameters():
        ...
        for attr in ['_forward_hooks', '_backward_hooks', '_grad_sample', 'summed_grad']:
...
class BiTFedSAFTLClientEpsilon(FedSAFTLClient):
    """Client that uses make_private_with_epsilon for DP-SGD on LoRA A matrices."""
    ...
    def __init__(self, client_id, model, device, config, privacy_mechanism=None):
        ...
        if self.use_dp and self.aggregation_method == 'fedsa_shareA_dp':
            ...
            if OPACUS_AVAILABLE:
                ...
                self.dp_target_epsilon = config.get('privacy', {}).get('epsilon', 8.0)
                if self.dp_target_epsilon is None:
                    raise ValueError("privacy.epsilon must be specified when using epsilon-target DP")
                self.dp_target_delta = config.get('privacy', {}).get('delta', 1e-5)
                self.dp_max_grad_norm = config.get('privacy', {}).get('max_grad_norm', 1.0)
                self.dp_poisson_sampling = config.get('privacy', {}).get('poisson_sampling', False)
                self.privacy_engine_attached = False
                self._private_loader = None
                print(f"Client {client_id}: Opacus epsilon-target DP initialized (ε={self.dp_target_epsilon}, δ={self.dp_target_delta})")
            else:
    ...
    def _attach_privacy_engine_with_epsilon(self, dataloader, epochs):
        if not hasattr(self, 'privacy_engine') or self.privacy_engine is None:
            raise RuntimeError("Privacy engine is not initialized")

        if self.dp_target_epsilon is None:
            raise ValueError("Target epsilon is not set")

        self.model, self.dp_optimizer, private_loader = self.privacy_engine.make_private_with_epsilon(
            module=self.model,
            ...
            poisson_sampling=self.dp_poisson_sampling
        )
        self.privacy_engine_attached = True
        self._private_loader = private_loader
        print(f"Client {self.client_id}: Attached Opacus with make_private_with_epsilon (ε={self.dp_target_epsilon}, δ={self.dp_target_delta}, epochs={epochs})")
    ...
    def train(self, dataloader, training_config):
        ...
        if (self.use_dp and self.aggregation_method == 'fedsa_shareA_dp' and
            ...
            dataloader = self._attach_privacy_engine_with_epsilon(dataloader, epochs)

        if self.use_dp and self.aggregation_method == 'fedsa_shareA_dp' and self._private_loader is not None:
            dataloader = self._private_loader

        dataset_size = len(dataloader.dataset)
        self.local_data_size = dataset_size
        ...
        if self.use_dp and self.aggregation_method == 'fedsa_shareA_dp' and self.privacy_engine_attached:
            try:
                epsilon = self.privacy_engine.accountant.get_epsilon(delta=self.dp_target_delta)
            except Exception:
                epsilon = self.dp_target_epsilon
            update['privacy_analysis'] = {
                'epsilon': epsilon,
                'target_epsilon': self.dp_target_epsilon,
                'delta': self.dp_target_delta,
                'max_grad_norm': self.dp_max_grad_norm,
                'batch_size': self.config['data'].get('batch_size', 128),
                'dataset_size': self.local_data_size,
                'method': 'Opacus DP-SGD (make_private_with_epsilon)'
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='FedSA-LoRA BiT with epsilon-target DP (A matrices only)')
    parser.add_argument('--config', type=str, default='configs/experiment_configs_non_iid/bit_tff_cifar100_epsilon.yaml',
    ...
    if not config_path.exists():
        ...
        print("Creating default configuration for epsilon-target DP...")
        ...
        default_config = {
            ...
            'experiment': {
                'name': 'BiT_TFF_CIFAR100_Epsilon',
                'output_dir': 'experiments/quickstart_bit_tff_epsilon'
            },
            'reproducibility': {
                'deterministic': False
            }
        ...
        config_path = configs_dir / "bit_tff_cifar100_epsilon.yaml"
    ...
    privacy_cfg = config.setdefault('privacy', {})
    privacy_cfg.setdefault('enable_privacy', True)
    privacy_cfg.setdefault('epsilon', 8.0)
    privacy_cfg.setdefault('delta', 1e-5)
    privacy_cfg.setdefault('max_grad_norm', 0.5)
    privacy_cfg.setdefault('poisson_sampling', False)
    ...
    print("=" * 80)
    print("FedSA-LoRA BiT Federated Learning with epsilon-target DP (A matrices only)")
    print("=" * 80)
    ...
    if privacy_enabled:
        print(f"Privacy Method: Opacus make_private_with_epsilon (ε target)")
    print("=" * 80)
    ...
    temp_client = BiTFedSAFTLClientEpsilon(0, initial_model, device, config)
    ...
    base_experiment_dir = Path(config.get('experiment', {}).get('output_dir', 'experiments/quickstart_bit_tff_epsilon'))
    ...
    date_bit_suffix = f"{current_date}BiTε"
    ...
    if slack_notifier:
        config_for_notification = {
            'experiment': {'name': f'BiT TFF QuickStart ε ({config["model"]["model_name"]})'},
    ...
    for i, client_id in enumerate(train_client_ids):
        ...
        if privacy_enabled:
            privacy_mechanism = DifferentialPrivacy(
                epsilon=config['privacy'].get('epsilon', 8.0),
        ...
        client = BiTFedSAFTLClientEpsilon(
            i,
            client_model,
            device,
            config,
    ...
    print("Starting BiT federated training with epsilon-target DP...")
    print("=" * 80)
    ...
    for round_idx in round_pbar:
        ...
        if is_eval_round:
            print("\nEvaluating personalized models (epsilon-target DP)")
        ...
        if config.get('privacy', {}).get('enable_privacy', False):
            ...
            if epsilons:
                ...
                print(f"\n  === Privacy Analysis (Opacus make_private_with_epsilon) ===")
                print(f"  ε (avg): {avg_eps:.4f}, ε (max): {max_eps:.4f}")
                print(f"  Target ε: {config['privacy'].get('epsilon', 8.0)}")
        ...
        round_pbar.set_postfix({
            ...
            'test_acc': f'{avg_personalized_acc:.2f}%' if avg_personalized_acc is not None else 'N/A',
            'best': f'{best_accuracy:.2f}%',
            'εmax': f'{current_epsilon:.2f}' if current_epsilon is not None else 'n/a'
        })
    ...
    print("\n" + "=" * 80)
    print("BiT Federated Learning Complete (epsilon-target DP)!")
    print("=" * 80)
    ...
    if slack_notifier:
        config_for_notification = {
            'experiment': {
                'name': f'BiT TFF QuickStart ε ({config["model"]["model_name"]})',
    ...
    if best_accuracy > 20:
        print("🎉 BiT federated transfer learning successful with epsilon-target DP!")
    ...
    try:
        ...
        if webhook_url:
            try:
                slack_notifier = SlackNotifier(webhook_url)
                config_for_notification = {
                    'experiment': {'name': 'BiT TFF QuickStart ε (ERROR)'}
                }
