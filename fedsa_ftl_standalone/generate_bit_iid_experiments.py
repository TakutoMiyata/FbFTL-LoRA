#!/usr/bin/env python3
"""
Generate BiT IID experiment configuration files
IID environment: data_split=iid, baseline performance measurement
"""
...
BASE_CONFIG = {
    ...
    'experiment': {
        'name': 'BiT_IID_Experiment',
        'output_dir': 'experiments/bit_iid',
        'save_history': True,
        'save_model': True,
        'log_interval': 10,
        'use_wandb': False,
        'wandb_project': 'bit-iid',
        'wandb_entity': None
    },
    'reproducibility': {
        'deterministic': False
...
def generate_experiment_configs():
    """Generate all IID experiment configurations"""
    ...
    base_dir = Path('configs/experiment_configs_bit_iid')
    ...
    for model, fraction in product(models, client_fractions):
        ...
        filename = f"{model}_iid_{fraction_str}_fedavg.yaml"
        output_dir = base_dir / 'fedavg'

        config['experiment'] = BASE_CONFIG['experiment'].copy()
        config['experiment']['name'] = f"BiT_IID_{model}_f{fraction:.1f}_FedAvg"
        config['experiment']['output_dir'] = f'experiments/bit_iid/fedavg/{model}_f{fraction:.1f}'
    ...
    for model, fraction, rank in product(models, client_fractions, lora_ranks):
        ...
        fraction_str = f"f{fraction:.1f}".replace('.', '')
        filename = f"{model}_iid_{fraction_str}_r{rank}_fedsa_lora.yaml"
        output_dir = base_dir / 'fedsa_lora'

        config['experiment'] = BASE_CONFIG['experiment'].copy()
        config['experiment']['name'] = f"BiT_IID_{model}_f{fraction:.1f}_r{rank}_FedSA_LoRA"
        config['experiment']['output_dir'] = f'experiments/bit_iid/fedsa_lora/{model}_f{fraction:.1f}_r{rank}'
    ...
    for model, fraction, rank, epsilon in product(models, client_fractions, lora_ranks, epsilon_values):
        ...
        fraction_str = f"f{fraction:.1f}".replace('.', '')
        filename = f"{model}_iid_{fraction_str}_r{rank}_eps{int(epsilon)}_fedsa_lora_dp.yaml"
        output_dir = base_dir / 'fedsa_lora_dp'

        config['experiment'] = BASE_CONFIG['experiment'].copy()
        config['experiment']['name'] = f"BiT_IID_{model}_f{fraction:.1f}_r{rank}_eps{int(epsilon)}_FedSA_LoRA_DP"
        config['experiment']['output_dir'] = f'experiments/bit_iid/fedsa_lora_dp/{model}_f{fraction:.1f}_r{rank}_eps{int(epsilon)}'
...
def save_experiments(experiments):
    ...
    print("=" * 80)
    print("Generating BiT IID Experiment Configurations")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    print("\n🎉 IID experiment configurations generated successfully!")
