...
CUSTOM DP VERSION:
This version uses custom DPOptimizer instead of Opacus PrivacyEngine for memory efficiency.
- Memory efficient: No grad_sample buffers (Opacus issue)
- Supports 'fedsa_custom_dp' aggregation method
- Still provides accurate epsilon tracking using Opacus RDPAccountant
- DP applied only to A matrices (B matrices use regular SGD)

Usage:
    python quickstart_bit_tff_custom_dp.py --config configs/config.yaml
...
Config example:
    ...
    federated:
      aggregation_method: 'fedsa_custom_dp'  # Use custom DP (not 'fedsa_shareA_dp')
"""
...
from dp_utils import WeightedFedAvg, DPOptimizer, create_dp_optimizer
...
class BiTFedSAFTLClient(FedSAFTLClient):
    ...
    def __init__(self, client_id, model, device, config, privacy_mechanism=None):
        ...
        if self.use_dp and self.aggregation_method == 'fedsa_custom_dp':
            ...
            # Use custom DPOptimizer instead of Opacus
            self.dp_optimizer = None  # Will be created in train() with dataset_size
            self.custom_dp_enabled = True
            self.optimizer = None
            print(f"Client {client_id}: Custom DP-SGD initialized (memory efficient)")

        # Opacus-based DP (original implementation)
        elif self.use_dp and self.aggregation_method == 'fedsa_shareA_dp':
            self.A_params = list(model.get_A_parameter_groups())
            self.B_params = list(model.get_B_parameter_groups())
            cls_params = []
            if hasattr(model, 'head'):
    ...
    def train(self, dataloader, training_config):
        """Train the model with A-only DP-SGD (via custom DPOptimizer or Opacus) and non-DP B+classifier"""
        self.model.train()
        self.local_data_size = len(dataloader.dataset)

        # Initialize custom DPOptimizer if needed
        if self.use_dp and self.aggregation_method == 'fedsa_custom_dp':
            if self.dp_optimizer is None:
                ...
                print(f"Client {self.client_id}: Custom DPOptimizer created for dataset size {self.local_data_size}")

        # Opacus-based DP (original)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (data, target) in enumerate(dataloader):
                ...
                elif self.use_dp and self.aggregation_method == 'fedsa_shareA_dp' and self.privacy_engine_attached:
        ...
        # Custom DP privacy analysis
        if self.use_dp and self.aggregation_method == 'fedsa_custom_dp':
            privacy_analysis = self.dp_optimizer.get_privacy_analysis()
            delta = self.config.get('privacy', {}).get('delta', 1e-5)

            # Use Opacus epsilon if available, otherwise custom approximation
            epsilon = privacy_analysis.get('opacus_epsilon', privacy_analysis.get('custom_epsilon', 0.0))
            ...
            update['privacy_analysis'] = {
                ...
                'dataset_size': self.local_data_size,
                'method': 'Custom DP-SGD (A-only, memory efficient)',
                'steps_taken': privacy_analysis.get('steps_taken', 0),
                'sampling_ratio': privacy_analysis.get('sampling_ratio', 0.0),
                'full_analysis': privacy_analysis
            }

            print(f"  Privacy spent: ε={epsilon:.4f} (δ={delta})")

        # Opacus DP privacy analysis
        elif self.use_dp and self.aggregation_method == 'fedsa_shareA_dp' and self.privacy_engine_attached:
            delta = self.config.get('privacy', {}).get('delta', 1e-5)
...
def main():
    ...
    for round_idx in round_pbar:
        ...
        if aggregation_method not in ['fedsa_shareA_dp', 'fedsa', 'fedsa_custom_dp']:
        ...
        if config.get('privacy', {}).get('enable_privacy', False):
            ...
            if privacy_analyses:
                ...
                if epsilons:
                    ...
                    # Get method from first analysis
                    method = privacy_analyses[0].get('method', 'DP-SGD')

                    print(f"\n  === Privacy Analysis ({method}) ===")
                    ...
                    if 'noise_multiplier' in privacy_analyses[0]:
                        noise_mult = privacy_analyses[0]['noise_multiplier']
                        max_grad_norm = privacy_analyses[0]['max_grad_norm']
                        actual_noise_std = noise_mult * max_grad_norm
                        print(f"  Noise multiplier: {noise_mult}")
                        print(f"  Max grad norm: {max_grad_norm}")
                        print(f"  Actual noise std: {actual_noise_std:.4f}")
