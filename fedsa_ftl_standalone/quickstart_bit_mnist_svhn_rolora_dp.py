...
from shadow_training import get_shadow_decoder, flatten_delta_dict
...
def ensure_rolora_defaults(config):
    defense_cfg = config.setdefault('defense', {})
    rolora_cfg = defense_cfg.setdefault('rolora_dp', {})
    model_rank = config.get('model', {}).get('lora', {}).get('r', 8)
    rolora_cfg.setdefault('enabled', False)
    rolora_cfg.setdefault('seed', 1337)
    rolora_cfg.setdefault('rank', model_rank if model_rank else 8)


def clone_param_dict(param_dict, device='cpu'):
    return {name: tensor.detach().to(device) for name, tensor in param_dict.items()}


def compute_delta_from_base(current_params, base_params):
    """Compute ΔA relative to shared base parameters."""
    delta = {}
    for name, tensor in current_params.items():
        base_tensor = base_params[name]
        delta[name] = tensor.detach().cpu() - base_tensor
    return delta


def rotate_delta_dict(delta_dict, rotation_matrix):
    rotated = apply_rolora_dp(delta_dict, rotation_matrix)
    return {name: tensor.detach().cpu() for name, tensor in rotated.items()}


def rotate_payload_delta(payload, rotation_matrix):
    rotated_payload = dict(payload)
    rotated_payload['delta_A_params'] = rotate_delta_dict(payload['delta_A_params'], rotation_matrix)
    return rotated_payload


def run_svd_dlg_gradient_attack(payload, config, gradient_cfg, device, global_A_params=None):
    """Execute SVD + DLG attack pipeline for a single payload."""
    delta_dict = {name: tensor.to(device) for name, tensor in payload['delta_A_params'].items()}

    batch_images = payload.get('batch_images')
    if batch_images is None:
        raise ValueError("Payload missing 'batch_images' for gradient attack.")
    if not torch.is_tensor(batch_images):
    ...
    batch_labels = payload.get('batch_labels')
    ...
    attack_model = make_bit_model_with_lora(config).to(device)
    if global_A_params:
        try:
            attack_model.set_A_parameters({name: tensor.to(device) for name, tensor in global_A_params.items()})
        except AttributeError:
            pass

    num_classes = payload.get('num_classes') or config.get('model', {}).get('num_classes', 10)
    initial_images = None
    shadow_cfg = gradient_cfg.get('shadow', {})
    if shadow_cfg.get('enabled'):
        try:
            decoder = get_shadow_decoder(config, device, shadow_cfg, global_A_params)
            decoder = decoder.to(device).eval()
            delta_vec = flatten_delta_dict({name: tensor.detach().cpu() for name, tensor in delta_dict.items()})
            with torch.no_grad():
                init_tensor = decoder(delta_vec.unsqueeze(0).to(device))
            if init_tensor.dim() == batch_images.dim() + 1:
                initial_images = init_tensor[0]
            else:
                initial_images = init_tensor
            if initial_images.dim() == batch_images.dim() - 1:
                initial_images = initial_images.unsqueeze(0)
            if initial_images.shape[0] < batch_images.shape[0]:
                repeat_times = (batch_images.shape[0] + initial_images.shape[0] - 1) // initial_images.shape[0]
                initial_images = initial_images.repeat(repeat_times, 1, 1, 1)[:batch_images.shape[0]]
            elif initial_images.shape[0] > batch_images.shape[0]:
                initial_images = initial_images[:batch_images.shape[0]]
            initial_images = initial_images.to(device, dtype=batch_images.dtype)
        except Exception as shadow_err:
            print(f"    Shadow decoder generation failed: {shadow_err}")
            initial_images = None

    attack_settings = {
        'attack_lr': payload.get('lr') or gradient_cfg.get('attack_lr', 0.1),
        'attack_steps': gradient_cfg.get('optimization_steps', 200),
        'data_reg': gradient_cfg.get('l2_regularizer', 1e-4),
        'tv_reg': gradient_cfg.get('tv_regularizer', 1e-5),
        'grad_weight': gradient_cfg.get('grad_weight', 1.0),
        'label_weight': gradient_cfg.get('label_weight', 1e-3),
        'num_classes': num_classes,
        'optimize_labels': gradient_cfg.get('optimize_labels', True),
        'initial_images': initial_images,
    }

    dlg_result, _, _ = dlg_refinement(
        attack_model,
        delta_dict,
        batch_images=batch_images,
        batch_labels=batch_labels,
        attack_config=attack_settings,
        device=device,
    )

    return dlg_result.to_dict()


def compute_a_recon_metrics(a_recon_records, config, a_recon_cfg):
    if not a_recon_cfg.get('enabled'):
        return None, None

    per_client_metrics = {}
    summary_accumulator = {}

    for client_id, record in a_recon_records.items():
        observed_list = record.get('observed', [])
        true_list = record.get('true', [])
        if not observed_list or not true_list:
            continue

        recon = reconstruct_A_from_noisy_updates(
            observed_list,
            method=a_recon_cfg.get('method', 'svd'),
            attack_config=a_recon_cfg,
        )
        metrics = evaluate_A_similarity(recon, true_list[-1])
        per_client_metrics[str(client_id)] = metrics
        for key, value in metrics.items():
            summary_accumulator.setdefault(key, []).append(value)

    if not per_client_metrics:
        return None, None

    mean_metrics = {key: sum(values) / len(values) for key, values in summary_accumulator.items()}
    return mean_metrics, per_client_metrics
...
def main():
    ...
    gradient_cfg.setdefault('tv_regularizer', 1e-5)
    gradient_cfg.setdefault('grad_weight', 1.0)
    gradient_cfg.setdefault('label_weight', 1e-3)
    gradient_cfg.setdefault('svd_topk', 1)
    shadow_attack_cfg = gradient_cfg.setdefault('shadow', {})
    shadow_attack_cfg.setdefault('enabled', False)
    shadow_attack_cfg.setdefault('dataset', config.get('data', {}).get('dataset_name', 'mnist'))
    shadow_attack_cfg.setdefault('num_samples', 64)
    shadow_attack_cfg.setdefault('batch_size', config.get('data', {}).get('batch_size', 64))
    shadow_attack_cfg.setdefault('client_lr', config.get('training', {}).get('lr', 0.05))
    shadow_attack_cfg.setdefault('decoder_hidden', 1024)
    shadow_attack_cfg.setdefault('decoder_lr', 1e-3)
    shadow_attack_cfg.setdefault('decoder_epochs', 3)
    shadow_attack_cfg.setdefault('cache_key', None)

    a_recon_cfg = privacy_attack_cfg.setdefault('A_reconstruction', {})
    a_recon_cfg.setdefault('enabled', True)
    a_recon_cfg.setdefault('method', 'svd')
    a_recon_cfg.setdefault('rounds_used', None)
    a_recon_cfg.setdefault('refine_steps', 0)

    ensure_rolora_defaults(config)
    rolora_cfg = config.setdefault('defense', {}).get('rolora_dp', {})
    rolora_enabled = rolora_cfg.get('enabled', False)
    rolora_context = None
    if rolora_enabled:
        context_rank = rolora_cfg.get('rank') or config.get('model', {}).get('lora', {}).get('r', 8) or 8
        rolora_context = RoLoRAContext(
            base_seed=int(rolora_cfg.get('seed', 1337)),
    ...
    a_recon_records = {client_id: {'observed': [], 'true': []} for client_id in range(len(clients))} if a_recon_cfg.get('enabled') else None
    ...
    for round_idx in round_pbar:
        ...
        rotation_matrix = rolora_context.matrix_for_round(round_idx) if rolora_enabled and rolora_context else None
        round_base_A = None
        current_a_recon_metrics = None
        if hasattr(server, 'global_A_params') and server.global_A_params:
            round_base_A = clone_param_dict(server.global_A_params, device='cpu')
        ...
        for client_idx in selected_clients:
            ...
            if round_base_A is not None:
                true_delta_cpu = compute_delta_from_base(client_result['A_params'], round_base_A)
            else:
                true_delta_cpu = {name: tensor.detach().cpu().clone() for name, tensor in client_result['A_params'].items()}

            if rolora_enabled and rotation_matrix is not None:
                rotated_delta = rotate_delta_dict(true_delta_cpu, rotation_matrix)
                client_result['rolora_rotated_delta'] = rotated_delta
                observed_delta_cpu = rotated_delta
            else:
                observed_delta_cpu = {name: tensor.clone() for name, tensor in true_delta_cpu.items()}

            if a_recon_records is not None:
                record = a_recon_records[client_idx]
                record['true'].append({name: tensor.clone() for name, tensor in true_delta_cpu.items()})
                record['observed'].append({name: tensor.clone() for name, tensor in observed_delta_cpu.items()})
        ...
        client_update_map = {client_id: client_updates[idx] for idx, client_id in enumerate(selected_clients)}
        ...
        if is_eval_round:
            ...
            for client_idx in selected_clients:
                ...
                if membership_cfg.get('enabled'):
                    ...
                    attack_model = make_bit_model_with_lora(config).to(device)
                    client_update = client_update_map.get(client_idx, {})
                    client_A_params = client_update.get('A_params') if client_update else None
                    if client_A_params:
                        attack_model.set_A_parameters(client_A_params)
                    elif hasattr(server, 'global_A_params') and server.global_A_params:
                        attack_model.set_A_parameters(server.global_A_params)
                    comprehensive_grad_sample_cleanup(attack_model, verbose=False)
            ...
            if a_recon_records is not None:
                mean_metrics_step, per_client_metrics_step = compute_a_recon_metrics(a_recon_records, config, a_recon_cfg)
                if mean_metrics_step:
                    print("  A-Reconstruction Mean (current rounds):")
                    for key, value in mean_metrics_step.items():
                        print(f"    {key}: {value:.4f}")
                    current_a_recon_metrics = {
                        'mean': mean_metrics_step,
                        'per_client': per_client_metrics_step,
                    }
        ...
        if gradient_attack_results:
            mse_values = [metric.get('reconstruction_mse') for metric in gradient_attack_results if metric.get('reconstruction_mse') is not None]
            label_values = [metric.get('label_accuracy') for metric in gradient_attack_results if metric.get('label_accuracy') is not None]
            ssim_values = [metric.get('ssim') for metric in gradient_attack_results if metric.get('ssim') is not None]
            psnr_values = [metric.get('psnr') for metric in gradient_attack_results if metric.get('psnr') is not None]
            avg_gradient_recon_mse = sum(mse_values) / len(mse_values) if mse_values else None
            avg_gradient_label_acc = sum(label_values) / len(label_values) if label_values else None
            avg_gradient_ssim = sum(ssim_values) / len(ssim_values) if ssim_values else None
            avg_gradient_psnr = sum(psnr_values) / len(psnr_values) if psnr_values else None
        else:
            avg_gradient_recon_mse = None
            avg_gradient_label_acc = None
            avg_gradient_ssim = None
            avg_gradient_psnr = None
        ...
        if rolora_enabled and rotation_matrix is not None and round_base_A is not None:
            client_A_params = []
            for update in client_updates:
                rotated_delta = update.get('rolora_rotated_delta')
                if rotated_delta is None:
                    raise RuntimeError("Missing rotated delta for RoLoRA aggregation.")
                recovered_delta = aggregate_with_inverse_rotation([rotated_delta], rotation_matrix)
                recovered_params = {}
                for name, delta_tensor in recovered_delta.items():
                    recovered_params[name] = round_base_A[name] + delta_tensor.cpu()
                client_A_params.append(recovered_params)
        else:
        ...
        if avg_gradient_recon_mse is not None:
            label_str = f"{avg_gradient_label_acc:.3f}" if avg_gradient_label_acc is not None else "n/a"
            ssim_str = f"{avg_gradient_ssim:.3f}" if avg_gradient_ssim is not None else "n/a"
            psnr_str = f"{avg_gradient_psnr:.2f}" if avg_gradient_psnr is not None else "n/a"
            print(f"  Avg Gradient Attack MSE: {avg_gradient_recon_mse:.4e}, Label Acc: {label_str}, SSIM: {ssim_str}, PSNR: {psnr_str}")
        ...
        if current_a_recon_metrics is not None:
            round_result['A_reconstruction_metrics'] = current_a_recon_metrics
        ...
        if avg_gradient_recon_mse is not None:
            ...
            round_result['avg_gradient_attack_ssim'] = avg_gradient_ssim
            round_result['avg_gradient_attack_psnr'] = avg_gradient_psnr
    ...
    gradient_ssim_values = [
        r.get('avg_gradient_attack_ssim') for r in results['rounds']
        if r.get('avg_gradient_attack_ssim') is not None
    ]
    if gradient_ssim_values:
        results['summary']['avg_gradient_attack_ssim_mean'] = sum(gradient_ssim_values) / len(gradient_ssim_values)
        results['summary']['avg_gradient_attack_ssim_last'] = gradient_ssim_values[-1]

    gradient_psnr_values = [
        r.get('avg_gradient_attack_psnr') for r in results['rounds']
        if r.get('avg_gradient_attack_psnr') is not None
    ]
    if gradient_psnr_values:
        results['summary']['avg_gradient_attack_psnr_mean'] = sum(gradient_psnr_values) / len(gradient_psnr_values)
        results['summary']['avg_gradient_attack_psnr_last'] = gradient_psnr_values[-1]

    if a_recon_records is not None:
        mean_metrics, per_client_metrics = compute_a_recon_metrics(a_recon_records, config, a_recon_cfg)
        if mean_metrics:
            results['summary']['A_reconstruction_mean'] = mean_metrics
            results['summary']['A_reconstruction_per_client'] = per_client_metrics
            a_payload = {
                'observed': {str(k): v['observed'] for k, v in a_recon_records.items()},
                'true': {str(k): v['true'] for k, v in a_recon_records.items()},
                'config': config,
            }
            recon_payload_path = experiment_dir / f'a_reconstruction_payload_{date_bit_suffix}.pt'
            torch.save(a_payload, recon_payload_path)
            print(f"A-reconstruction payload saved to: {recon_payload_path}")
            print("A-reconstruction mean metrics:")
    ...
    try:
        ...
        for round_data in results['rounds']:
            ...
            if 'avg_gradient_attack_mse' in round_data:
                ...
                row_data['gradient_attack_ssim'] = round_data.get('avg_gradient_attack_ssim')
                row_data['gradient_attack_psnr'] = round_data.get('avg_gradient_attack_psnr')
    ...
    if 'avg_gradient_attack_ssim_mean' in summary_data:
        print(f"Gradient Attack SSIM → mean: {summary_data['avg_gradient_attack_ssim_mean']:.3f}, "
              f"last round: {summary_data['avg_gradient_attack_ssim_last']:.3f}")
    if 'avg_gradient_attack_psnr_mean' in summary_data:
        print(f"Gradient Attack PSNR → mean: {summary_data['avg_gradient_attack_psnr_mean']:.2f}, "
              f"last round: {summary_data['avg_gradient_attack_psnr_last']:.2f}")
    if 'A_reconstruction_mean' in summary_data:
        print("A-Reconstruction Mean Metrics:")
        for key, value in summary_data['A_reconstruction_mean'].items():
            print(f"  {key}: {value:.4f}")
    if 'avg_gradient_attack_label_accuracy_mean' in summary_data:
        print(f"Gradient Attack Label Accuracy → mean: {summary_data['avg_gradient_attack_label_accuracy_mean']:.3f}, "
              f"last round: {summary_data['avg_gradient_attack_label_accuracy_last']:.3f}")
