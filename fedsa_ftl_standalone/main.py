...
def main(config):
    ...
    temp_client = FedSAFTLClient(0, create_model(config['model']), device)
    ...
    for round_idx in range(config['federated']['num_rounds']):
        ...
        round_stats = server.federated_round(client_updates, test_dataloader)
        ...
        if 'test_loss' in round_stats:
            print(f"  Test Loss: {round_stats['test_loss']:.4f}")
            print(f"  Test Accuracy: {round_stats['test_accuracy']:.2f}%")
